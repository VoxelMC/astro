...
import { after, describe as describeNode, before, it } from 'node:test';
import * as cheerio from 'cheerio';
import { loadFixture, isWindows } from './test-utils.js';
import testAdapter from './test-adapter.js';

const describe = isWindows ? describeNode.skip : describeNode;
...
describe('Experimental Content Collections cache - render()', () => {
	describe('Build - SSG', () => {
		...
		it('Includes CSS for rendered entry', async () => {
			...
			// Renders content
			assert.strictEqual($('ul li').length, 3);
		...
		it('Excludes CSS for non-rendered entries', async () => {
			...
			// Excludes styles
			assert.strictEqual($('link[rel=stylesheet]').length, 0);
		});
		...
		it('De-duplicates CSS used both in layout and directly in target page', async () => {
			...
			$('link[rel=stylesheet]').each((_, linkEl) => {
				...
				assert.strictEqual(set.has(href), false);
				set.add(href);
			});

			$('style').each((_, styleEl) => {
				const textContent = styleEl.children[0].data;
				assert.strictEqual(set.has(textContent), false);
				set.add(textContent);
			});
		});
		...
		it.skip('Includes component scripts for rendered entry', async () => {
			...
			assert.strictEqual($('script[data-is-inline]').length, 1);
		});
		...
		it('Applies MDX components export', async () => {
			...
			assert.strictEqual(h2.attr('data-components-export-applied'), 'true');
		});
	});
	...
	describe('Build - SSR', () => {
		...
		it('getCollection should return new instances of the array to be mutated safely', async () => {
			...
			assert.strictEqual($('li').first().text(), 'With Layout Prop');

			request = new Request('http://example.com/');
			response = await app.render(request);
			html = await response.text();
			$ = cheerio.load(html);
			assert.strictEqual($('li').first().text(), 'Hello world');
		});
	});
	...
	describe('Dev - SSG', () => {
		...
		it('Includes CSS for rendered entry', async () => {
			...
			// Includes styles
			assert.strictEqual($('head > style').length, 1);
		...
		it('Supports layout prop with recursive getCollection() call', async () => {
			...
			const body = $('body');
			assert.strictEqual(body.attr('data-layout-prop'), 'true');

			const h1 = $('h1');
			assert.strictEqual(h1.length, 1);
			assert.strictEqual(h1.text(), 'With Layout Prop');

			const h2 = $('h2');
			assert.strictEqual(h2.length, 1);
			assert.strictEqual(h2.text(), 'Content with a layout prop');
		});
	});
});
